...
class _CreateLoanPageState extends State<CreateLoanPage> {
  ...
  void _onItemTapped(int index) {
    ...
    void _createLoanProfile() {
      double totalAmountPaid = terms.fold(0.0, (sum, term) => sum + term["amount"]);

      final loan = Loan(
        loanId: 'some_unique_id',
        ...
        isPaid: totalAmountPaid >= (double.tryParse(_loanAmountController.text) ?? 0.0),
        createdAt: DateTime.now(),
        terms: terms,
        amountPaid: totalAmountPaid, // Total amount paid from terms
      );

      final borrower = {
        'borrowerId': 'some_unique_borrower_id',
        'name': 'John Doe',
      };

      // Pass the loan object and borrower data to the LoanProfilePage
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => LoanProfilePage(
            borrower: borrower,  // Pass the borrower data
            loan: loan,          // Pass the loan data
          ),
        ),
      );
    }
